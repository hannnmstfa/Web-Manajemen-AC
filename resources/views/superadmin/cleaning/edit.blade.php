    ...
    <div class="container mt-4">
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h5>Edit Data Cleaning</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('cleaning.update', $proses->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <!-- Tanggal Planning -->
                    <div class="mb-3">
                        <label class="form-label">Tanggal Planning</label>
                        <input type="date" name="tgl_planing" class="form-control" value="{{ $proses->tgl_planing }}"
                            required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="ac_id" id="select2" class="form-control" required>
                            <option value="" disabled>-- Pilih Nama AC --</option>
                            @if ($ac->isEmpty())
                                <option disabled class="text-center fst-italic">Belum Ada Data AC</option>
                            @else
                                <x-select2 />
                                @foreach ($ac as $ac)
                                    <option value="{{ $ac->id }}" {{$ac->id == $proses->ac_id ? 'selected' : ''}}>
                        ...
                        <select name="vendor_id" id="select_2" class="form-control" required>
                            ...
                            @else
                                ...
                                @foreach ($vendor as $vendor)
                                    <option value="{{ $vendor->id }}" {{$vendor->id == $proses->vendor_id ? 'selected' : ''}}>
                                        {{ $vendor->nama_vendor }}
                                    </option>
                                @endforeach
                            @endif
